...
@section('styles')
    ...
    <style>
        ...
        #preview {
            width: 100%;
            height: 300px;
            /* Fixed height for better camera display */
            border: 1px solid #ddd;
            background-color: #f8f9fa;
            /* Visual feedback when camera is loading */
        }
            ...
            <div class="bg-white p-6 rounded-lg shadow-md">
                ...
                <form action="{{ url('/obatmasuk') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label for="BesarPotongan" class="block text-sm font-medium text-gray-700">Besar
                                Potongan</label>
                            <input type="text" id="BesarPotongan" name="BesarPotongan"
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-black">
                        </div>

                        <div>
                            <label for="PotCash" class="block text-sm font-medium text-gray-700">Potongan Cash</label>
                            <input type="text" id="PotCash" name="PotCash"
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-black">
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex space-x-2">
                                <input type="text" id="id_obat" name="id_obat"
                                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-black"
                                    placeholder="Masukkan kode atau scan barcode">
                                <button id="startScan" type="button"
                                    class="mt-4 bg-blue-500 text-white px-4 py-2 rounded-md">Scan</button>
                            </div>
                        </div>
                        <!-- Scanner container -->
                        <div id="scannerContainer" class="mt-4 hidden">
                            ...
                            <button type="button" id="closeScan"
                                class="mt-2 bg-red-500 text-white px-4 py-2 rounded-md">Tutup Kamera</button>
                        </div>
                    </div>
                    <div class="mt-6">
                        ...
                        <tr>
                            ...
                            <td>
                                <button class="action-btn edit-btn" data-bs-toggle="modal"
                                    data-bs-target="#editObatMasukModal" data-id="{{ $item->NoDetBeli }}">
                                    <i class='bx bx-edit'></i>
                                </button>

                                <form action="{{ url('/det_transaksi_pembelian/' . $item->NoDetBeli) }}" method="POST"
